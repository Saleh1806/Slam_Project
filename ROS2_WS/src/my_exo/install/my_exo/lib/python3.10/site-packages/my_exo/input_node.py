    ...
    def keyboard_input(self):
        while True:
            try:
                ...
                if 1 <= selected_topic <= 4:
                    msg = Int32()
                    msg.data = selected_topic
                    self.publisher_.publish(msg)
                    self.get_logger().info(f"Published selected topic: {selected_topic}")
                else:
                    self.get_logger().warning("Invalid input. Please enter a number between 1 and 4.")
            except ValueError:
                self.get_logger().warning("Invalid input. Please enter a valid number.")
