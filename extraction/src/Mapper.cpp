...
#define ANGLE_PENALTY_GAIN 0.2
  ...
  // ========================================================================
  // FONCTION CUDA OPTIMISÉE: Calcul des distances avec GPU
  // ========================================================================
  // Calcule les distances² entre tous les scans et une pose de référence
      ...
      std::vector<kt_double> &distances)
  {
    ...
    // === ÉTAPE 1: Préparer les données pour GPU ===
    std::vector<float> poses_x, poses_y;
    poses_x.reserve(scans.size());
    poses_y.reserve(scans.size());

    // Extraire les coordonnées X et Y de tous les scans
    ...
    // === ÉTAPE 2: Allocer espace pour résultats ===
    std::vector<float> gpu_distances(scans.size());

    // === ÉTAPE 3: Appeler le kernel CUDA ===
    // Cette fonction:
    // - Copie les données du CPU vers le GPU
    // - Lance le kernel pour calculer les distances en parallèle
    ...
    // === ÉTAPE 4: Vérifier les erreurs CUDA ===
    if (err != cudaSuccess)
    {
      // Fallback sur CPU en cas d'erreur
      std::cerr << "CUDA Error in ComputeDistancesGPU: "
                << cudaGetErrorString(err) << std::endl;
      ComputeDistancesCPU(refPose, scans, distances);
      return;
    }

    // === ÉTAPE 5: Convertir résultats en doubles ===
    distances.clear();
  ...
  // ========================================================================
  // FONCTION FALLBACK: Calcul des distances sur CPU
  // ========================================================================
  // Utilisée comme fallback si GPU n'est pas disponible ou en cas d'erreur
  // ========================================================================
  ...
  {
    distances.clear();
    distances.reserve(scans.size());

    for (const auto &scan : scans)
    {
      Pose2 scanPose = scan->GetReferencePose(
          m_pMapper->m_pUseScanBarycenter->GetValue());
      kt_double dx = refPose.GetX() - scanPose.GetX();
      kt_double dy = refPose.GetY() - scanPose.GetY();
      distances.push_back(dx * dx + dy * dy);
    }
  }

  // ========================================================================
  // FONCTION OPTIMISÉE: Trouver le scan le plus proche
  // ========================================================================
  // Utilise GPU pour calculer rapidement les distances
  // puis cherche le minimum sur CPU (opération rapide)
  // ========================================================================
      ...
      const Pose2 &rPose) const
  {
    ...
    // === OPTIMISATION GPU ===
    // Calculer les distances² en parallèle sur GPU
    std::vector<kt_double> distances;
  ...
  {
    std::map<int,
             Vertex<LocalizedRangeScan> *>::iterator v1 = m_Vertices[pSourceScan->GetSensorName()].find(pSourceScan->GetStateId());
    std::map<int,
             Vertex<LocalizedRangeScan> *>::iterator v2 = m_Vertices[pTargetScan->GetSensorName()].find(pTargetScan->GetStateId());
