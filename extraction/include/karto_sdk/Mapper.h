...
namespace karto
{
  ...
  public:
    ...
    inline void RemoveVertex(const Name &rName, const int &idx)
    {
      ...
      {
        std::cout << "RemoveVertex: Failed to remove vertex " << idx << " because it doesnt exist in m_Vertices." << std::endl;
      }
    }
  ...
  private:
    ...
    /**
     * CUDA-optimized: Compute squared distances between all scans and a reference pose using GPU
     * Falls back to CPU if CUDA is not available or if an error occurs
     * @param refPose reference pose
     * @param scans vector of scans
     * @param distances output vector of squared distances
     */
    ...
    /**
     * CPU fallback: Compute squared distances between all scans and a reference pose
    ...
    /**
     * Finds the closest scan in the vector to the given pose (GPU-optimized)
     * @param rScans
     * @param rPose
     */
