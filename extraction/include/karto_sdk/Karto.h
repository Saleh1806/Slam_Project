...
void load(Archive& ar, const unsigned int version) {
    std::vector<std::string> temp_PointReadings;
    std::vector<std::string> temp_UnfilteredPointReadings;

    // Charger les valeurs dans des vecteurs temporaires
    ar & BOOST_SERIALIZATION_NVP(m_OdometricPose);
    ar & BOOST_SERIALIZATION_NVP(m_CorrectedPose);
    ar & BOOST_SERIALIZATION_NVP(m_BarycenterPose);
    ar & BOOST_SERIALIZATION_NVP(temp_PointReadings);
    ar & BOOST_SERIALIZATION_NVP(temp_UnfilteredPointReadings);
    ar & BOOST_SERIALIZATION_NVP(m_BoundingBox);
    ar & BOOST_SERIALIZATION_NVP(m_IsDirty);
    ar & BOOST_SERIALIZATION_BASE_OBJECT_NVP(LaserRangeScan);

    // Filtrer les chaînes "inf" et "-inf" dans les vecteurs temporaires
    filterOutInf(temp_PointReadings);
    filterOutInf(temp_UnfilteredPointReadings);

    // Copier les valeurs filtrées dans les membres de la classe
    m_PointReadings = std::move(temp_PointReadings);
    m_UnfilteredPointReadings = std::move(temp_UnfilteredPointReadings);
}
private:
...
private:
  ...
  Pose2 m_CorrectedPose;
    ...
    // Méthode pour filtrer les chaînes "inf" et "-inf"
    void filterOutInf(std::vector<std::string>& values) {
        auto end = std::remove_if(values.begin(), values.end(), [](const std::string& value) {
